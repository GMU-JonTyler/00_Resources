...
"""Implementation of (Mannstein et al 1999) algorithm for contrail detection."""
...
def get_regional_gradient_mask(t_12um, t_12um_stddev,
                               config):
...
def mannstein_one_angle_mask(features,
                             config,
                             degrees):
  ...
  if 'regional_gradient_max_scale' in config:
    regional_gradient_mask = get_regional_gradient_mask(t_12um,
  ...
  line_mask = ((detected_lines > config['normalized_sum_threshold']) &
...
def mannstein_contrail_mask(features,
                            config):
